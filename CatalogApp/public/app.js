    ...
    mainGrid.innerHTML = subiecteCatalog.map(materie => {
        ...
        return `
         <div class="bg-white rounded-2xl shadow-sm shadow-[0_4px_16px_rgba(0,0,0,0.02)] border border-gray-100 p-5 min-h-[145px] flex flex-col justify-between hover:-translate-y-[2px] transition-transform cursor-default">
            ...
            <div class="flex justify-between items-start mb-6 border-b border-gray-50 pb-3">
               <h4 class="font-bold text-[14px] text-gray-800 pt-0.5 tracking-tight">${materie}</h4>
               <span class="bg-indigo-50 text-indigo-500 border border-indigo-100/50 text-xs px-2.5 py-0.5 font-bold rounded-full">${badgeTXT}</span>
            </div>

            <!-- Rand bule Note albe bordurate rotunjit-->
            <div class="flex gap-2.5 flex-wrap flex-grow content-center items-center justify-start min-h-[44px]">
                ${bulUI}
            </div>
...
// Eleu - Actiunea Trimite Acoperit Pntu Acasa. Put Direct 
async function finishHw(tem_uid) {
    if(!confirm('Profesorul de materie va asista decizia că te ai pregătit pentru clasa viitoare, Finalizezi?')) return;
    await fetch(`${API}/teme/${tem_uid}/done`, {method:'PUT'});
    initStudent(); 
}
